...
function ProfileContainer(props) {
  ...
  const formatLastSeenDate = (lastSeenDate) => {
    ...
    if (timeDiff < 2) {
      ...
      const day =
        lastSeen.getDate() < 10 ? `0${lastSeen.getDate()}` : lastSeen.getDate();
      const month =
        lastSeen.getMonth() < 9
          ? `0${lastSeen.getMonth() + 1}`
          : lastSeen.getMonth() + 1;
      const year = lastSeen.getFullYear();
      const hours =
        lastSeen.getHours() < 10
          ? `0${lastSeen.getHours()}`
          : lastSeen.getHours();
      const minutes =
        lastSeen.getMinutes() < 10
          ? `0${lastSeen.getMinutes()}`
          : lastSeen.getMinutes();
    ...
    <div className="user-profile-container container-fluid">
      <div className="row">
        ...
        {userProfileData.status == 1 ? (
          ...
          <div className="col-sm-12 col-md-7">
            {isUSerProfileOwner() ? (
              <span>
                <Link
                  to="/about-us#how-to-be-experienced"
                  style={{ textDecoration: "none ", color: "black" , textDecoration:"underline" }}
                >
                  Nasıl Deneyimli Üye Olabilirim ?
                </Link>
              </span>
            ) : (
              <span>
                {" "}
                Kullanıcı Şimdilik deneyim vermeyi tercih etmemektedir{" "}
              </span>
            )}
          </div>
        )}
        ...
        <div className="row py-4">
          ...
          <div className="col-sm-12 col-md-7">
            <ProfileComments formattedAvarageRate={formattedAvarageRate} />
          </div>
        </div>
      </div>
    </div>
